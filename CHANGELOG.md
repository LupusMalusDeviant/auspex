...
- **SafeSearch, per profile and per time window.** A device profile can send
  ...
  from — Google (all of its country domains, via the public suffix list),
  YouTube moderate and strict, Bing, DuckDuckGo, Yandex, Pixabay. A schedule
  ...
  The answer carries the target's address and not just a CNAME — a stub
  resolver does not follow the chain itself, and a CNAME-only answer means the
  page does not load. HTTPS records (type 65) are redirected as well, because
  Encrypted Client Hello and alternative addresses live there; MX, NS and the
  rest pass through untouched. A block beats the redirect: whoever blocked
  YouTube meant blocked.

  A misspelt provider makes the start fail rather than quietly not filtering
  — the same rule as for `block_services`, and for a worse reason: nothing
  visibly breaks, so nobody notices.

- **Listen addresses can be marked `optional`.** A listener that does not come
  up terminates Auspex — deliberately, because a resolver answering only on
  ...
  *worse* than the crash it replaces — a crash heals itself through the
...
- **DNS rebinding protection, as a finding rather than a silent drop.** An
  ...
  tenth detector turns it into a warning — or a hard finding when several
...
- **The chain: detect, act, let go.** A finding that names a device gets a
  button that puts its profile into quarantine — a new resolver policy that
...
- **A new page, "Programs".** Which program on a device talked to which
  ...
  than dropped — on that page they are the interesting number.
...
- **The language header is `X-Auspex-Language`.** The old
  `X-Auspex-Sprache` is still accepted — an extension already installed in a
...
- **Interfaces at the seams.** The services that leave the process — resolver,
  router, rule files, origin database, stored settings — are now reached
...
earlier — the kind of break that is invisible until you have to say, for every
name, what it means.
...
- **`compose.yml` set `Geo__Stadt`,** while the option is called `City`. The
  city lookup could not be switched off — `GEO_STADT=false` never arrived.
...
- **`setup.ps1` did not parse.** `"$asWhom:"` reads to PowerShell as a scoped
  variable reference, and the whole file stopped parsing — the one failure a
...
- **Timestamps from SQLite were rendered as local time without conversion.**
  ...
  nothing at all, and the implicit conversion to `DateTimeOffset` is worse — it
...
hand-written with `RenameTable`/`RenameColumn` — the one EF scaffolded wanted
...
speak the new field names while an installed build speaks the old ones — the
...
the new build is enough — the address and the token come along.
